...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // 내용 초기값 설정
    const initialContent = contentInput.value || '내용을 입력하세요';
    try {
        const htmlContent = DOMPurify.sanitize(marked.parse(initialContent));
        previewContent.innerHTML = htmlContent;
    } catch (error) {
        console.error('Markdown 변환 중 오류:', error);
        previewContent.textContent = initialContent;
    }
    ...
    if (imgInput.getAttribute('value')) {
        previewImg.src = imgInput.getAttribute('value');
        previewImg.classList.remove('hidden');
    }
    
    // 비디오 초기값 설정
    const videoInput = document.getElementById('video');
    if (videoInput.getAttribute('value')) {
        previewVideo.src = videoInput.getAttribute('value');
...
function insertMarkdown(startTag, endTag) {
    ...
    // 마크다운 삽입 후 미리보기 업데이트
    const markdownText = textarea.value || '내용을 입력하세요';
    try {
        const htmlContent = DOMPurify.sanitize(marked.parse(markdownText));
        previewContent.innerHTML = htmlContent;
    } catch (error) {
